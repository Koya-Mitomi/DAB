class IncomeAmountsController < ApplicationController
  ...
  def index
    if params["date(2i)"].present?
      @income_amounts = current_user.income_amounts.where(date: Time.new(params["date(1i)"], params["date(2i)"], params["date(3i)"]).all_month, expenditure_id: params[:format]).order(:date)
    else
